...
def decompose_sql(clients, question, type, config):   
    ...
    prompt_for_decompose = f"""I will now give you a math problem. The type of problem is {type}. Please break the problem into the FEWEST solvable steps (2–5).
    ...
    Each step should logically depend on the result of the previous step.
    Do not generate independent or parallel steps.
    The reasoning should follow a step-by-step, linear execution process.
    """
    # prompt_for_decompose =f"""
    #         I will give you a problem. Break it into the FEWEST solvable steps (2–5).
    #         Each step must be a single question that directly advances the solution.
    #         No definitions, no restating the problem, no off-topic steps, no parallel steps.
    #         Answer format (copy exactly; one step per line, end each with "?"):
    #         To solve the question "<goal>", we need to know:
    #         "1. <step 1 question>",
    #         "2. <step 2 question>",
    #         "3. <step 3 question>".
    ...
    result = askLLM(clients, Query, tokens_path=config['tokens_path'], model=config['decompose_MODEL'], temperature=0.3)
    return result
